...
for(const locale of ['','zh-hant/','zh-hans/']){
 ...
 for(const width of [1440,390])for(const theme of ['light','dark']){
  ...
  const frame=page.locator('.media-player iframe');assert.equal(await frame.count(),1);assert.match(await frame.getAttribute('src'),/^https:\/\/www.youtube-nocookie.com\/embed\/OuoJdw_wRbI\?/);assert.equal(new URL(await frame.getAttribute('src')).searchParams.get('autoplay'),'0');assert.equal(await frame.getAttribute('loading'),'lazy');assert.equal(await frame.getAttribute('referrerpolicy'),'strict-origin-when-cross-origin');assert(!(await frame.getAttribute('allow')).includes('autoplay'));assert.match(await frame.getAttribute('title'),/Short teaching showcase/);
