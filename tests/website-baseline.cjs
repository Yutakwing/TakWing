// Read-only local browser baseline. Third-party services are blocked; no student data is written.
...
const main = ['index', 'about', 'research', 'teaching', 'writing', 'media', 'resources', 'collaborate'];
...
(async () => {
 ...
 } finally {fs.writeFileSync(process.env.BASELINE_REPORT || '/tmp/takwing-phase1-browser.json',JSON.stringify(report,null,2));await browser.close();}
