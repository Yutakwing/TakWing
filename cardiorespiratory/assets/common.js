(() => {
  ...
  function initPlacementGame(config){
    initialiseTheme(); const svg=qs("[data-game-board]"); svg.innerHTML=boardMarkup(config.view); const markerEl=qs("[data-marker]",svg); const debugLayer=qs("[data-debug-targets]",svg); const completedLayer=qs("[data-completed-sites]",svg); const state={stage:0,score:0,attempts:0,startedAt:null,timer:null,x:config.start.x,y:config.start.y,dragging:false}; const total=config.points.length; const tolerance=()=>GAME_MODE==="assessment"?config.tolerance*0.82:config.tolerance;
    function render(){markerEl.setAttribute("transform",`translate(${state.x} ${state.y})`);updateStatus(state,total);setText("[data-instruction]",config.points[state.stage]?.instruction||config.completeTitle);}
    ...
    qs("[data-check]").addEventListener("click",()=>{startClock(state,total);state.attempts+=1;const bone=boneFeedback();if(bone){playIncorrectTone();setText("[data-feedback]",bone);updateStatus(state,total);return;}const point=config.points[state.stage];const distance=Math.hypot(state.x-point.x,state.y-point.y);if(distance<=tolerance()){state.score+=Math.max(10,Math.round(100/total-(distance/tolerance())*4));const dot=document.createElementNS("http://www.w3.org/2000/svg","circle");dot.setAttribute("cx",point.x);dot.setAttribute("cy",point.y);dot.setAttribute("r",9);dot.setAttribute("class","completed-site");completedLayer.append(dot);state.stage+=1;playCorrectChime();setText("[data-feedback]",point.correct||"Correct. Continue to the matching site on the other side.");if(state.stage>=total)complete();else render();}else{playIncorrectTone();const horizontal=state.x<point.x?"right":"left";const vertical=state.y<point.y?"lower":"higher";setText("[data-feedback]",GAME_MODE==="practice"?`Not quite. Move ${horizontal} and ${vertical}, then check again.`:"Placement not accepted. Try again.");}updateStatus(state,total);});
    qs("[data-hint]").addEventListener("click",()=>{if(GAME_MODE==="assessment")return;setText("[data-feedback]",config.points[state.stage]?.hint||config.generalHint);});
    function restart(){stopClock(state);Object.assign(state,{stage:0,score:0,attempts:0,startedAt:null,timer:null,x:config.start.x,y:config.start.y});completedLayer.replaceChildren();qs("[data-completion]").hidden=true;setText("[data-feedback]",config.startFeedback);window.PhysioSkillsProgress?.resetCompletion();render();}
