(() => {
  ...
  function initPlacementGame(config){
    ...
    qs("[data-check]").addEventListener("click",()=>{if(state.stage>=total)return;startClock(state,total);state.attempts+=1;const tutorStage=state.stage;const report=(result,error,distance)=>{const adapter=window.SkillsTutorAdapters?.[config.gameId];if(adapter)window.PhysioSkillsProgress?.recordFeedback(config.gameId,adapter(tutorStage,state,result,error,distance,state.stage>=total));};const bone=boneFeedback();if(bone){report("incorrect",bone.includes("scapula")?"over-scapula":"over-spine",0);playIncorrectTone();setText("[data-feedback]",bone);updateStatus(state,total);return;}const isPairStart=config.pairedSequence&&state.stage%2===0&&state.stage+1<total;const candidateIndexes=isPairStart?[state.stage,state.stage+1]:[state.stage];let matchedIndex=candidateIndexes[0];let point=points[matchedIndex];let distance=Math.hypot(state.x-point.x,state.y-point.y);for(const index of candidateIndexes.slice(1)){const candidate=points[index];const candidateDistance=Math.hypot(state.x-candidate.x,state.y-candidate.y);if(candidateDistance<distance){matchedIndex=index;point=candidate;distance=candidateDistance;}}if(distance<=tolerance()){if(isPairStart&&matchedIndex!==state.stage){[points[state.stage],points[matchedIndex]]=[points[matchedIndex],points[state.stage]];point=points[state.stage];}const completedStage=state.stage;state.score+=Math.max(10,Math.round(100/total-(distance/tolerance())*4));const dot=document.createElementNS("http://www.w3.org/2000/svg","circle");dot.setAttribute("cx",point.x);dot.setAttribute("cy",point.y);dot.setAttribute("r",9);dot.setAttribute("class","completed-site");completedLayer.append(dot);state.stage+=1;playCorrectChime();const feedback=config.pairedSequence?completedStage%2===0?"Correct. Now compare the equivalent site on the opposite side.":"Correct. This side-to-side pair is complete.":point.correct||"Correct. Continue to the matching site on the other side.";setText("[data-feedback]",feedback);if(state.stage>=total)complete();else render();report("correct","none",distance);}else{const otherSite=points.some((p,index)=>!candidateIndexes.includes(index)&&Math.hypot(state.x-p.x,state.y-p.y)<=tolerance());report("incorrect",otherSite?"wrong-comparison-sequence":Math.abs(state.y-point.y)>tolerance()?"wrong-level":Math.abs(state.x-360)<Math.abs(point.x-360)?"too-medial":"too-lateral",distance);playIncorrectTone();const horizontal=state.x<point.x?"right":"left";const vertical=state.y<point.y?"lower":"higher";setText("[data-feedback]",GAME_MODE==="practice"?`Not quite. Move ${horizontal} and ${vertical}, then check again.`:"Placement not accepted. Try again.");}updateStatus(state,total);});
    qs("[data-hint]").disabled = GAME_MODE === "assessment";
