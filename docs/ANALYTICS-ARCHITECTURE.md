...
**Software integration: COMPLETE. Cloudflare Web Analytics: PENDING TOKEN.**
...
3. Paste the complete snippet as the JSON `snippet` string in `site-analytics.config.json`; retain its dashboard attributes. The validator accepts one external non-blocking script from the official beacon path, optional integrity/crossorigin and dashboard data attributes. A changed official snippet format may require a reviewed validator update, never guessing or weakening it.
...
The current tests verify only the intercepted official-origin script insertion and exclusion logic. No genuine RUM delivery or dashboard data has been verified because no snippet exists.
...
Three generated Privacy pages explain the pending status, future public analytics scope, game/student separation, theme preferences, FormSubmit and third-party embeds/fonts/tutor. New privacy copy remains English with TRANSLATION REQUIRED on Chinese routes. There is no generic cookie banner, legal-compliance claim or absolute-anonymity claim.
...
Reviewed for this implementation on 19–20 September 2026. Dashboard access and genuine property ownership were not verified.
